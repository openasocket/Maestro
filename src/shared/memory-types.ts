...
export interface MemoryConfig {
	 ...
	 * when the agent's train of thought shifts to a different domain — default true */
	enableLivePersonaShift: boolean;
...
export type PendingContextSource =
	| 'cross-agent'
	| 'new-experience'
	| 'skill-update'
	| 'monitoring'
...
export interface MemoryStats {
	...
	totalLinks: number;
	/** Number of experiences that have been promoted to rules */
	promotedCount: number;
	/** Number of cross-project promotion candidates detected */
	crossProjectCandidates: number;
	/** Aggregated search performance metrics (populated from ring buffer) */
	searchPerformance?: SearchPerformanceMetrics;
}

/** Aggregated search performance metrics from the in-memory ring buffer. */
export interface SearchPerformanceMetrics {
	/** Total searches recorded in the buffer */
	totalSearches: number;
	/** Average search latency in milliseconds */
	avgLatencyMs: number;
	/** P95 search latency in milliseconds */
	p95LatencyMs: number;
	/** Fraction of searches that returned at least one result (0-1) */
	hitRate: number;
	/** Average number of results returned per search */
	avgResultCount: number;
	/** Searches using embedding vs fallback (keyword-only) */
	embeddingSearches: number;
	fallbackSearches: number;
	/** Breakdown of searches by strategy */
	byStrategy: Record<string, number>;
	/** Average top similarity score across searches that had embedding results */
