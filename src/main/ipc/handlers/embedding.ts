	...
	ipcMain.handle(
		...
		createIpcDataHandler(
			...
			async (providerId: EmbeddingProviderId, config: EmbeddingProviderConfig) => {
				// Merge the stored OpenAI API key into the config — the renderer
				// never holds the actual key (security: secret not exposed back),
				// so we must inject it from the settings store before activation.
				let mergedConfig = config;
				if (providerId === 'openai' && settingsStore) {
					const memoryConfig = settingsStore.get('memoryConfig') as
						| { embeddingProvider?: EmbeddingProviderConfig }
						| undefined;
					const storedKey = memoryConfig?.embeddingProvider?.openai?.apiKey;
					if (storedKey) {
						const defaults = {
				...
				logger.debug(
					`Switching to provider "${providerId}" with config: ${JSON.stringify(sanitizeConfig(mergedConfig))}`,
					LOG_CONTEXT
				);
				await embeddingRegistry.switchProvider(providerId, mergedConfig);
