...
export class TransformersJsProvider implements EmbeddingProvider {
	...
	/**
	 * Load @xenova/transformers via native dynamic import().
	 * Uses Function() wrapper to prevent tsc from rewriting import() to require()
	 * under CommonJS — @xenova/transformers is ESM-only.
	 * Static so tests can spy on TransformersJsProvider.loadTransformers.
	 */
	static async loadTransformers(): Promise<typeof import('@xenova/transformers')> {
		return Function('return import("@xenova/transformers")')() as Promise<
	...
	async initialize(config: EmbeddingProviderConfig): Promise<void> {
		...
		try {
			...
			const { pipeline, env } = await TransformersJsProvider.loadTransformers();
