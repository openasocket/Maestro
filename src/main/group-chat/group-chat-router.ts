...
export async function routeModeratorResponse(
	...
	if (processManager && agentDetector && mentions.length > 0) {
		...
		for (const participantName of mentions) {
			...
			const geminiParticipantDirArgs = buildGeminiWorkspaceDirArgs(agent, participant.agentId, [
				cwd,
				groupChatFolder,
				os.homedir(),
			]);
...
export async function respawnParticipantWithRecovery(
	...
	const geminiRecoveryDirArgs = buildGeminiWorkspaceDirArgs(agent, participant.agentId, [
		cwd,
		groupChatFolder,
		os.homedir(),
	]);
