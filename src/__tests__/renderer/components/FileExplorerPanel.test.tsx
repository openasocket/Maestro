...
describe('FileExplorerPanel', () => {
	...
	describe('VIBES View AI Blame', () => {
		function openFileContextMenu(container: HTMLElement, name = 'package.json') {
			const fileItem = Array.from(container.querySelectorAll('[data-file-index]')).find((el) =>
				el.textContent?.includes(name)
			);
			expect(fileItem).toBeTruthy();
		...
		it('does not show View AI Blame option for folders', () => {
			...
			openFileContextMenu(container, 'src');
			expect(screen.queryByText('View AI Blame')).not.toBeInTheDocument();
		});

		it('calls onViewAIBlame with the file path and closes the menu when clicked', () => {
			const onViewAIBlame = vi.fn();
			const { container } = render(
				<FileExplorerPanel {...defaultProps} onViewAIBlame={onViewAIBlame} />
			);
			openFileContextMenu(container);
			fireEvent.click(screen.getByText('View AI Blame'));
			expect(onViewAIBlame).toHaveBeenCalledWith(expect.stringContaining('package.json'));
			expect(screen.queryByText('View AI Blame')).not.toBeInTheDocument();
		});
	});
});
