/**
 * useSwipeGestures - multi-directional touch swipe detection
 *
 * Detects horizontal and vertical swipe gestures with configurable distance /
 * velocity thresholds, optional offset tracking for drag animations, and
 * direction locking. Returns touch handlers to spread on a target element.
 *
 * Ported from the legacy mobile app at `src/web/hooks/useSwipeGestures.ts`. The
 * only adaptation is the `GESTURE_THRESHOLDS` import, which now points at the
 * shared renderer touch primitives (`src/renderer/utils/touch.ts`).
 *
 * Note: `handleTouchMove` calls `e.preventDefault()` once it locks to a
 * horizontal swipe, so attach these handlers only to elements where suppressing
 * native horizontal scroll is intended (e.g. screen-edge drawer zones or a
 * modal backdrop), never to a scrollable content region.
 */

import { useCallback, useRef, useState } from 'react';
import { GESTURE_THRESHOLDS } from '../../utils/touch';
...
/**
 * Return type for useSwipeGestures.
 */
