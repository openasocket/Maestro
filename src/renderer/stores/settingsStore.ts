...
export const useSettingsStore = create<SettingsStore>()((set, get) => {
	...
	return {
		...
		setVibesAttestationCosign: (value) => {
			set({ vibesAttestationCosign: value });
			window.maestro.settings.set('vibesAttestationCosign', value);
		},

		setVibesAttestationSubmit: (value) => {
			set({ vibesAttestationSubmit: value });
			window.maestro.settings.set('vibesAttestationSubmit', value);
		},

		setVibesInsightsEnabled: (value) => {
			set({ vibesInsightsEnabled: value });
...
export async function loadAllSettings(): Promise<void> {
	try {
		...
		if (allSettings['vibesAttestationCosign'] !== undefined)
			patch.vibesAttestationCosign = allSettings['vibesAttestationCosign'] as boolean;

		if (allSettings['vibesAttestationSubmit'] !== undefined)
			patch.vibesAttestationSubmit = allSettings['vibesAttestationSubmit'] as boolean;

		if (allSettings['vibesInsightsEnabled'] !== undefined)
			patch.vibesInsightsEnabled = allSettings['vibesInsightsEnabled'] as boolean;
