...
function InjectionActivitySection({
	...
	// Memory detail popover: tracks which memory ID is expanded and its fetched content
	...
	const handleMemoryIdClick = useCallback(
		async (id: string) => {
			...
			try {
				const res = await window.maestro.memory.findById(id);
				if (res.success) {
					setMemoryDetail(res.data ?? null);
				}
			} catch {
				// Non-critical
			} finally {
														...
														<div className="ml-2 space-y-1">
															<div className="flex flex-wrap gap-1">
																{event.memoryIds.slice(0, 10).map((id, mi) => (
																	<span
																		key={mi}
																		className="px-1 rounded font-mono cursor-pointer transition-colors"
																		style={{
																			backgroundColor:
																				expandedMemoryId === id
																		...
																		title={`Click to view details \u2022 Right-click to copy ID: ${id}`}
																		onClick={() => handleMemoryIdClick(id)}
																		onContextMenu={(e) => {
																			e.preventDefault();
																			...
																			notifyToast({
																				type: 'info',
																				title: 'Copied',
																				message: 'Memory ID copied to clipboard',
																			});
																		}}
																	>
																		{id.slice(0, 12)}
															...
															{/* Inline memory detail popover */}
															{expandedMemoryId && event.memoryIds.includes(expandedMemoryId) && (
																<div
																...
																>
																	{memoryDetailLoading ? (
																		<div style={{ color: theme.colors.textDim }}>Loading...</div>
																	) : memoryDetail ? (
																		<>
																			<div className="flex items-center gap-2 flex-wrap">
																				...
																				<span
																					...
																					style={{ color: theme.colors.textDim }}
																				>
																					scope: {memoryDetail.scope}
																				</span>
																				<span
																					className="text-[9px]"
																					style={{ color: theme.colors.textDim }}
																				>
																					confidence: {(memoryDetail.confidence * 100).toFixed(0)}%
																				</span>
																				<span
																					className="text-[9px]"
																					style={{ color: theme.colors.textDim }}
																				>
																					used {memoryDetail.useCount}x
																				</span>
																			</div>
																			<div
																		...
																		<div style={{ color: theme.colors.textDim }}>
																			Memory not found (may have been archived or deleted)
																		</div>
																	)}
																</div>
															)}
										...
										<div className="flex-1 min-w-0 pb-1">
											...
											{isExpanded && (
												...
												>
													...
													<div
														className="font-mono text-[9px] opacity-60 cursor-pointer hover:opacity-100 transition-opacity"
														title="Click to copy full memory ID"
														onClick={(e) => {
															e.stopPropagation();
															navigator.clipboard.writeText(evt.memoryId);
															notifyToast({
																type: 'info',
																title: 'Copied',
																message: 'Memory ID copied to clipboard',
