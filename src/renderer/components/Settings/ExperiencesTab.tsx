...
function ExperienceVisualizationSummary({
	...
	// Promoted items included in the list (rules with 'promoted:experience' tag)
	const promotedInList = useMemo(
		() =>
			experiences.filter((e) => e.type === 'rule' && e.tags.includes('promoted:experience')).length,
		[experiences]
	);
	const experienceOnlyCount = total - promotedInList;
	// Use the higher of stats-based count and list-based count (stats includes all scopes)
	const effectivePromotedCount = Math.max(promotedCount, promotedInList);
	...
	const normalCount = experienceOnlyCount - deviationCount;
		...
		<div className="rounded-lg border p-3 space-y-3" style={{ borderColor: theme.colors.border }}>
			...
			<div className="flex items-stretch gap-2">
				...
				<div className="flex-1 rounded p-2" style={{ backgroundColor: `${theme.colors.border}15` }}>
					...
					>
						<span className="font-bold">{experienceOnlyCount}</span>
						...
						<span
							className="font-bold"
							style={{ color: effectivePromotedCount > 0 ? '#22c55e' : undefined }}
						>
							{effectivePromotedCount}
						</span>
						<span className="text-[10px]" style={{ color: theme.colors.textDim }}>
							promoted
						</span>
...
export function ExperiencesTab({
	...
	const handleMovePromoteConfirm = useCallback(
		...
		[movePromoteMemory, projectPath, loadExperiences, onRefresh, onCuratorAction]
	);
	...
	const handlePromote = useCallback(
		...
		[loadPromotionCandidates, loadExperiences, onRefresh, onCuratorAction]
	);
...
function ExperienceReviewCard({
	...
	const isPromoted = experience.type === 'rule' && experience.tags.includes('promoted:experience');
	...
	return (
		...
		>
			...
			{!isPromoted && isPromotionCandidate && (
				<div
					className="flex items-center gap-1 text-[10px] font-medium"
					style={{ color: '#d4a017' }}
				>
