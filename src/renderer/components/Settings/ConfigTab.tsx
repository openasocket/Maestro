					...
					{storageOpen && (
						...
						>
							...
							<ConfigToggle
								...
								description="Inject new persona context mid-session when task focus changes"
								tooltip="When enabled, the system monitors chain-of-thought during agent execution and injects relevant persona memories when it detects a shift in task focus. Delivered via the live injection queue at the next prompt."
								checked={config.enableLivePersonaShift}
								onChange={(v) => onUpdateConfig({ enableLivePersonaShift: v })}
								theme={theme}
							/>

							<ConfigToggle
								label="Live Injection"
								description="Inject checkpoint memories mid-session at periodic intervals"
								tooltip="When enabled, the system periodically searches for relevant memories during active sessions and injects them at the next user prompt. Works alongside live persona shifting to keep agent context fresh."
								checked={config.enableLiveInjection}
								onChange={(v) => onUpdateConfig({ enableLiveInjection: v })}
								theme={theme}
							/>

							<ConfigToggle
								label="Cross-Agent Broadcast"
								description="Share memory injection results across active agents"
								tooltip="When enabled, memories injected into one agent session are broadcast to other active agents that share the same project context. Useful for multi-agent workflows where agents benefit from each other's memory context."
								checked={config.enableCrossAgentBroadcast}
								onChange={(v) => onUpdateConfig({ enableCrossAgentBroadcast: v })}
								theme={theme}
							/>
						</div>
					)}
