...
const OPENAI_MODELS = [
	{ value: 'text-embedding-3-small', label: 'text-embedding-3-small', cost: 0.02, maxDim: 1536 },
	{ value: 'text-embedding-3-large', label: 'text-embedding-3-large', cost: 0.13, maxDim: 3072 },
	{ value: 'text-embedding-ada-002', label: 'text-embedding-ada-002', cost: 0.1, maxDim: 1536 },
];
...
function getDimensionPresets(model: string): DimensionPreset[] {
	if (model === 'text-embedding-3-large') {
		return [
			{ label: 'Low', value: 384, description: '384d — fast, compact' },
			{ label: 'Medium', value: 1024, description: '1024d — balanced' },
			{ label: 'High', value: 3072, description: '3072d — full precision' },
		];
	}
	// text-embedding-3-small
	return [
		{ label: 'Low', value: 384, description: '384d — fast, compact' },
		{ label: 'Medium', value: 768, description: '768d — balanced' },
		{ label: 'High', value: 1536, description: '1536d — full precision' },
	];
}
...
export function EmbeddingProviderSettings({
	...
	// Which provider card the user clicked (may differ from activeProviderId before activation)
	const [selectedProviderId, setSelectedProviderId] = useState<EmbeddingProviderId | null>(null);
	...
	const activateProvider = useCallback(
		...
		[activeProviderId, embeddingConfig, onUpdateConfig, fetchStatus, handleReEmbedAll]
	);
	...
	const handleSelectProvider = useCallback(
		async (providerId: EmbeddingProviderId) => {
			...
			// For OpenAI without a key: just show settings panel, don't activate yet
			if (providerId === 'openai' && !hasOpenAIKey) return;

			await activateProvider(providerId);
		},
		[switching, hasOpenAIKey, activateProvider]
	);
	...
	const handleSetKey = useCallback(async () => {
		...
		try {
			...
			if (res.success) {
				...
				// Auto-activate OpenAI now that the key is set
				if (activeProviderId !== 'openai') {
					await activateProvider('openai');
				}
			}
		} catch {
			notifyToast({
	...
	}, [keyInput, activeProviderId, activateProvider]);
		...
		<div className="space-y-3">
			...
			<div className="grid grid-cols-4 gap-2">
				{PROVIDERS.map((provider) => {
					...
					const isSelected = provider.id === selectedProviderId && !isActive;
					...
					return (
						...
						>
							...
							{status?.error && (
								<div
									className="text-[9px] leading-tight mt-0.5 max-w-full truncate"
			...
			{/* Provider-Specific Settings — show for selected card (or active if none selected) */}
			{(selectedProviderId || activeProviderId) &&
				(() => {
					const visibleId = selectedProviderId ?? activeProviderId;
					return (
						<div
						...
						>
							...
							{(visibleId === 'transformers-js' || visibleId === 'xenova-onnx') &&
								activeProviderId === visibleId && (
									<TransformersSettings
										theme={theme}
										config={embeddingConfig}
										providerId={visibleId}
							...
							{/* Ollama */}
							{visibleId === 'ollama' && activeProviderId === 'ollama' && (
								<OllamaSettings
									theme={theme}
									config={embeddingConfig}
									models={ollamaModels}
							...
							{/* OpenAI — show when selected OR active (so user can enter key before activation) */}
							{visibleId === 'openai' && (
								<OpenAISettings
									theme={theme}
									config={embeddingConfig}
								...
								<div>
									...
									{reEmbedResult && (
										<div
											...
											style={{
												color:
													reEmbedResult.failed > 0 ? theme.colors.warning : theme.colors.success,
											}}
										>
	...
	return (
		<>
			...
			{openai.model.startsWith('text-embedding-3') &&
				(() => {
					const presets = getDimensionPresets(openai.model);
					const modelInfo = OPENAI_MODELS.find((m) => m.value === openai.model);
					const maxDim = modelInfo?.maxDim ?? 1536;
					const isPresetValue = presets.some((p) => p.value === openai.dimensions);
					return (
								...
								<div className="flex items-center gap-1.5">
									{presets.map((preset) => (
										<button
											...
											style={{
												borderColor:
													openai.dimensions === preset.value
														? theme.colors.accent
														: theme.colors.border,
												backgroundColor:
													openai.dimensions === preset.value
											...
											onClick={() =>
												onUpdateConfig({
													openai: { ...openai, dimensions: preset.value },
												})
											}
									...
									<input
										className="w-16 p-1 rounded border bg-transparent outline-none text-xs font-mono text-center"
										style={{
											borderColor: !isPresetValue ? theme.colors.accent : theme.colors.border,
											color: theme.colors.textMain,
										}}
										type="number"
										...
										onChange={(e) => {
											...
											if (val && val >= 64 && val <= maxDim) {
												onUpdateConfig({
													openai: { ...openai, dimensions: val },
												});
											}
										}}
										title={`Custom (64–${maxDim})`}
									/>
								</div>
								<div className="text-[10px]" style={{ color: theme.colors.textDim }}>
									{presets.find((p) => p.value === openai.dimensions)?.description ??
										`${openai.dimensions}d — custom`}
								</div>
							</div>
						</SettingField>
					);
